...
class SmsSentReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        ...
        try {
            ...
            val ok = SmsSender.isAcceptedByRadio(radioCode)
            ...
            if (SmsSender.isRetryableRadioError(radioCode) && formatIndex + 1 < candidates.size) {
                AppExecutors.sms.execute {
                    runCatching {
                        SmsSender.send(context.applicationContext, phone, message, recipientId, simSlot, formatIndex + 1)
                    }.onFailure { err ->
            ...
            if (!SmsSender.isHardRadioRefusal(radioCode)) {
                AppExecutors.sms.execute {
                    try {
                        SmsSender.send(
                        ...
                        report(context, recipientId, false, "SMS_FAILED", "sent resultCode=$radioCode")
                    }
                }
                return
            }
            ...
            val code = if (radioCode == SmsSender.RESULT_ERROR_LIMIT_EXCEEDED) "RATE_LIMIT" else "NO_SIM"
            if (radioCode == SmsSender.RESULT_ERROR_LIMIT_EXCEEDED) {
                StatusStore.pauseSendsUntil = System.currentTimeMillis() + 90_000
            }
            StatusStore.lastError = "SMS refusée (resultCode=$radioCode)"
            report(context, recipientId, false, code, "sent resultCode=$radioCode")
        } catch (err: Throwable) {
            JobGuard.complete(recipientId)
